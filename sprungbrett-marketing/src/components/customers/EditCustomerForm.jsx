...
export default function EditCustomerForm({ open, onOpenChange, customer, onUpdated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await Customer.update(customer.id, form);
      onUpdated?.();
      onOpenChange(false);
    } catch (err) {
      console.error("Kunde aktualisieren fehlgeschlagen:", err);
    } finally {
